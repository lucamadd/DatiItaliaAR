        ...
        void Update()
        {
            switch (m_State)
            {
                case State.AddImagesRequested:
                {
                    if (m_Images == null)
                    {
                            GUILayout.Label("No images to add.");
                        break;
                    }
                    ...
                    if (manager == null)
                    {
                            GUILayout.Label($"No {nameof(ARTrackedImageManager)} available.");
                        break;
                    }
                    ...
                    foreach (var image in m_Images)
                    {
                        ...
                        {
                                GUILayout.Label($"Image {image.name} must be readable to be added to the image library.");
                            break;
                        }
                    }
                    ...
                    if (manager.referenceLibrary is MutableRuntimeReferenceImageLibrary mutableLibrary)
                    {
                        ...
                        {
                                GUILayout.Label($"ScheduleAddImageJob threw exception: {e.Message}");
                        }
                    }
                    else
                    {
                            GUILayout.Label($"The reference image library is not mutable.");
                    }
